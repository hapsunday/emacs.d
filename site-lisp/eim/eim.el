...
(defun eim-terminate-translation ()
  ...
  ;;   (progn 
  ;;     (unless (eq nil eim-popup-tip-inst)
  ;;       (popup-delete eim-popup-tip-inst))
      ...
      (let ((message-log-max nil))
        (if eim-use-tooltip
            (let ((pos (string-match ": " eim-guidance-str)))
              ...
              (eim-choose-show-zhk eim-guidance-str))
              ;; (if (display-graphic-p)
              ;;     (eim-show-tooltip eim-guidance-str)
              ;;   (eim-show-popuptip eim-guidance-str)))
			  ;; (popup-tip eim-guidance-str))
...
(defface eim-popup-tip-face
  '((t (:background "khaki1" :foreground "black")))
  "Face for eim popup tip."
  :group 'eim)

(defun eim-choose-show-zhk (text)
  " Choose tip function by display-graphic-p  "
  (if (display-graphic-p)
      (eim-show-pos-tip text)
    (eim-show-popuptip text)))

(defun eim-show-pos-tip (text)
  " show popuptip text near cursor by pos-tip. by zhk"
  (let ((lines (delete "" (split-string (substring-no-properties text) " "))))
    (unwind-protect
        (progn 
          (pos-tip-show-no-propertize (mapconcat 'identity
                                                 (cons (car lines)
                                                       (cons (make-string
                                                              (apply 'max
                                                                     (mapcar
...
(defun eim-show-popuptip (text)
  "Show popuptip text near cursor. by zhk"
  (let ((lines (delete "" (split-string (substring-no-properties text) " ")))
        width height)
    (setq width (apply 'max (mapcar 'string-width lines))
          height (1+ (length lines)))
    (setq tip (popup-create (point) width height
                            :around t
                            :face 'eim-popup-tip-face))
    (setq lines (cons (car lines)
                      (cons (make-string width ?=) (cdr lines))))
    (unwind-protect
        (progn 
...
(defun eim-show-tooltip (text)
  ...
  (let ((pos (eim-frame-posn-at-point))
        ...
        (lines (delete "" (split-string (substring-no-properties text) " ")))
;        (lines (split-string text "\n"))
        width height left top temp-text)
    (setq width (apply 'max (mapcar 'string-width lines))
          height (1+ (length lines)))
    (setq lines (cons (car lines)
                      (cons (make-string width ?=) (cdr lines))))
    (setq temp-text (mapconcat 'identity lines "\n"))

    (setq width (* (frame-char-width) width)
          height (* (frame-char-height) height))
    ...
    (x-show-tip (propertize temp-text 'face 'eim-tooltip-face)
